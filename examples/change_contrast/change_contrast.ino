/*
 This example shows how you can change the contrast of the display without reseting your arduino
*/

#include <Nokia5110.h>

Nokia5110 lcd(7, 6, 5, 4, 3); // (PIN_SCE, PIN_RESET, PIN_DC, PIN_SDIN, PIN_SCLK)
// PIN_SDIN and PIN_SCLK should be driven by PWM enabled pins

boolean increase = true; // if true, contrast increases - if false, contrast decreases
byte contrast = 0xB0;

// this is awesome in bitmap form
char awesome[] =
{
	,
};

void setup(void)
{
}

void loop(void)
{
	lcd.setContrast(contrast);
	lcd.init(); // init the LCD

	lcd.clear();
	lcd.bitmap(awesome); // print bitmap array
	delay(500);

	if (increase)
	{
		contrast++;
	}
	else
	{
		contrast--;
	}

	if (contrast == 0xB0) // reached lowest contrast value
	{
		increase = true;
	}

	if (contrast == 0xBF) // reached highest contrast value
	{
		increase = false;
	}
}
